<div
    ...
    <div class="p-2 sm:p-3 md:p-4">
        <div class="text-xs sm:text-sm font-light mt-1 sm:mt-2 text-gray-900 dark:text-gray-100 line-clamp-2">
            {{ $product->name }}</div>
        <div class="flex items-center mb-1">
            ...
            @endfor
            <span
                class="ml-1 text-xs text-gray-500 dark:text-gray-400 hidden sm:inline">({{ number_format($rating, 1) }})</span>
        </div>
        <div class="flex items-center mb-2 flex-wrap">
            @if (isset($product->discount) && $product->discount > 0)
                ...
                @endphp
                <span
                    class="text-gray-400 dark:text-gray-500 text-xs line-through mr-1 sm:mr-2">{{ $product->currency }}
                    {{ number_format($originalPrice, 2) }}</span>
                <span class="text-red-500 dark:text-red-400 text-sm font-semibold mr-1 sm:mr-2">{{ $product->currency }}
                    {{ number_format($product->price, 2) }}</span>
                <span
                    class="bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-200 text-xs px-1 sm:px-2 py-0.5 rounded-full font-bold">-{{ round(($product->discount / $originalPrice) * 100) }}%</span>
            @else
        ...
        @if ($product->stock <= 0)
            <div class="text-red-500 dark:text-red-400 text-xs mt-1 sm:mt-2">Out of Stock</div>
        @elseif($product->stock <= 5)
            <div class="text-orange-500 dark:text-orange-400 text-xs mt-1 sm:mt-2">Only {{ $product->stock }} left in
                stock
            </div>
        @endif
    </div>
